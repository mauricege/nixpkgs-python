{
  ...
  in {
    lib.applyOverrides = overrides: pkg: let
    ...
    lib.mkPython = {
      ...
      infix =
        if sourceVersion.major == "2"
        then "2.7/"
        else "";
      overrideLDConfigPatch = path: pkg:
        pkg.override {
          noldconfigPatch = path;
        };
      replacePatch = name: patch: pkg:
        pkg.overrideAttrs (old: {
          patches =
            (lib.filter (elem:
              if builtins.isNull elem
              then true
              else !lib.hasSuffix name elem)
            old.patches)
            ++ [patch];
        });
    ...
    in
      (self.lib.applyOverrides overrides (pkgs.callPackage "${pkgs.path}/pkgs/development/interpreters/python/cpython/${infix}default.nix" ({
    ...
    checks = forAllSystems (
      system: let
        ...
        getRelease = callPackage: version: source:
          self.lib.mkPython {
        ...
        packages =
          pkgs.lib.mapAttrs (getRelease callPackage) self.lib.versions.releases
